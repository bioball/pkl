...
# Determine the project name based on the output .dylib argument
for arg in "$@"; do
  if [[ "$arg" == *.dylib ]]; then
...
for arg in $CC_ARGS; do
  if [[ "$arg" == *.dylib ]]; then
    # replace output dylib with our own, placed into our own output dir
    SHARED_CC_ARGS+=("${OUTPUT_PATH}/libpkl.dylib")
    ...
    # add symbols from pkl.o to exported_symbols.list
    SYMBOL_LIST_FILE=$(echo "$arg" | cut -d "," -f2-)
    echo "Adding symbols to $SYMBOL_LIST_FILE" >> "${LOG_FILE}"
    nm -g "$PKL_OBJECT_FILE" | grep ' T ' | awk '{print $3}' >> "$SYMBOL_LIST_FILE"
    SHARED_CC_ARGS+=("$arg")
  else
    SHARED_CC_ARGS+=("$arg")
  fi
...
echo "cc" "${SHARED_CC_ARGS[@]}" >> "$LOG_FILE"
cc "${SHARED_CC_ARGS[@]}"
