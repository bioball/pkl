...
tasks.processResources {
  ...
  filesMatching("org/pkl/gradle/PklPlugin.properties") {
...
tasks.processTestResources {
  val pklToolsShadowJar = project(":pkl-tools").tasks.named("shadowJar")
  dependsOn(pklToolsShadowJar)
  filesMatching("org/pkl/gradle/PklPlugin.properties") {
    filter<ReplaceTokens>(
      "tokens" to
        mapOf(
          "pklToolsPath" to
            pklToolsShadowJar.get().outputs.files.singleFile.toPath().toUri().toString()
        )
    )
  }
}
